...
<body>
	<div id="app">
		<form>
			<textarea rows="10" cols="100" v-model="jpql"></textarea>
			<br />
			<button type="button" @click="search">JPQL 실행</button>
		</form>
		<div v-if="error" class="msg">
			{{ error }} <br /> {{ error?.response?.data?.message }}
		</div>

		<table v-if="!error && (typeof list[0] == 'object')"> <!-- response가 객체일 경우 -->
			<thead>
				<tr>
		...
		<ul v-if="!error && (typeof list[0] != 'object')">  <!-- response가 객체가 아닌 value일 경우 -->
